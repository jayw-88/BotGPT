...
# page config
st.set_page_config(page_title="BotGPT")

if "chats" not in st.session_state:
    st.session_state.chats = []
if "system_instruction_change" not in st.session_state:
    st.session_state.system_instruction_change = (
        "You are a generalized helpful assistant that helps with users' tasks."
    )
...
if submit_button and user_input:
    ...
    if user_input.lower() == "m":
        ...
        st.session_state.console_output.append(
            "\n Welcome to Chat Management. What would you like to do? View"
            " Chats (V), Create a New Chat (N), or Delete a Chat (X)"
        )
        ...
        if user_chat.lower() == "v":
            ...
            if len(st.session_state.chats) == 0:
                st.session_state.console_output.append(
                    "You have no chats to view!"
                )
            else:
                st.session_state.console_output.append(
                    str(st.session_state.chats)
                )
            st.rerun()
    ...
    # Settings
        ...
        st.session_state.console_output.append("\nWelcome to Settings!")
        st.session_state.console_output.append(
            "Here you can give BotGPT certain instructions for his"
            " responses.\n Press 1 for a faster model and 2 for a more thorough"
            " model."
        )
        st.session_state.console_output.append(
            "Input your settings or press 'X' to exit."
        )
        if st.button("1"):
